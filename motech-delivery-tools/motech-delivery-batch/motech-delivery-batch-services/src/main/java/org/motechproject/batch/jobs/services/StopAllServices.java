...
public class StopAllServices implements Tasklet, InitializingBean {
    ...
    @Autowired
    public StopAllServices(@Qualifier("batchServiceProperties") Properties batchProperties) {
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        ...
        String buildFile = prefix(buildFilePath) + "operate.xml";
        String environment = batchProperties.getProperty("environment");
        String antFile = batchProperties.getProperty("ant.file");
        int returnCode = new AntTask(buildFile, buildFilePath, environment, antFile, true).run("stop.services");
        if (returnCode == 0)
